/*
 * occ_c.h — the public C product surface over Open CASCADE Technology.
 *
 * =============================================================================
 * HOW TO READ THIS API (literate guide)
 * =============================================================================
 *
 * Goal: callers write pure C (or any language FFI). They never include OCCT
 * headers, never link OCCT C++ types across the boundary, never need a C++
 * toolchain for application code.
 *
 * Mental model:
 *
 *   application / Luau / Wasm JS
 *           │  POD args + opaque handles + int status
 *           ▼
 *   this header (and occ_c_all.h for expansions)
 *           │
 *           ▼
 *   api/src/occ_c_*.cc  — thin maps: C args → a few OCCT calls → C results
 *           │
 *           ▼
 *   OCCT 7.9.3 (implementation detail only)
 *
 * Conventions (apply to every symbol below and in expansion modules):
 *
 *   1. Status is int: OCC_OK (0) or OCC_ERR_*. On failure call occ_last_error().
 *   2. Lengths are meters; angles are radians; topology indices are 1-based.
 *   3. Shapes (occ_shape_t) and meshes (occ_mesh_t) are opaque owned handles.
 *      Every successful *out shape/mesh is caller-owned → occ_shape_free /
 *      occ_mesh_free. Input handles are never freed by the callee.
 *   4. Exceptions never cross this boundary; they become OCC_ERR_EXCEPTION.
 *
 * Full expansion (session, construction, frames, route/pipe, patterns, holes,
 * measure, sweeps):  #include "occ_c_all.h"
 *
 * Walk the implementation starting at api/internal/occ_c_internal.hxx, then
 * the .cc that matches the feature you care about (route → occ_c_route.cc, …).
 */
...
/* Opaque BREP handle. Implementation is TopoDS_Shape* on the heap — callers
 * never see that type. */
typedef struct occ_shape_s* occ_shape_t;
/* Opaque triangle mesh for viewers. Free with occ_mesh_free. */
typedef struct occ_mesh_s*  occ_mesh_t;

/*
 * Return codes. Values 0–7 are frozen (baseline). 8+ are expansion codes.
 * Do not invent parallel #define error numbers in other headers — they will
 * collide (e.g. GEOM must be 15, not 8).
 */
typedef enum {
  OCC_OK                = 0,
  OCC_ERR_NULL_ARG      = 1,
...
/* Public shape taxonomy for bindings. Not raw OCCT TopAbs values.
 * Use occ_shape_type() to obtain these. */
typedef enum {
  OCC_SHAPE_UNKNOWN   = 0,
...
/* Clash outcome (out-parameter of occ_clash), not a function return code. */
typedef enum {
  OCC_CLASH_SEPARATED = 0, /* min distance > clearance */
  OCC_CLASH_CLEARANCE = 1, /* 0 < dist ≤ clearance */
  OCC_CLASH_INTERFERE = 2, /* touching / overlapping */
} occ_clash_status_t;

OCC_API const char* occ_version(void);
/* Thread-local last error string. Meaningful when the last call returned ≠ OK. */
...
/* ---- Primitives ---------------------------------------------------------- */
/* Axis-aligned box from origin with size (dx,dy,dz). */
OCC_API int occ_make_box(double dx, double dy, double dz, occ_shape_t* out);
/* Cylinder: base point (cx,cy,cz), axis direction, radius, height. */
...
/* ---- Booleans (non-consuming inputs; *out is a new shape) ---------------- */
...
/* ---- Rigid transforms (copy-on-write; input shape unchanged) ------------- */
...
/* ---- Triangle mesh for external viewers ---------------------------------- */
/* Vertices/normals/indices pointers remain valid until occ_mesh_free. */
